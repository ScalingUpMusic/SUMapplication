...
object MultiTagModel {
    ...
    def trainModel(tag: String, filie: org.apache.spark.rdd.RDD[String]) : (org.apache.spark.mllib.classification.SVMModel, String) = {
        ...
        val data_unscaled = filie.map(line => extractData(line, tag))
        if (verbose) {
    ...
    def main(args: Array[String]){
        ...
        val allSongData = sc.textFile(loadpath).collect()
        val models = topTagsList.map(tag => trainModel(tag, allSongData))

...
object MultiTagModel {
    ...
    def trainModel(tagLabel: (String, Array[Double]), songData: Array[org.apache.spark.mllib.linalg.Vector], sc: SparkContext) {
        ...
        val data_unscaled = sc.parallelize(labels zip songData)
        if (verbose) {
    ...
    def main(args: Array[String]){
        ...
        tagLabels.collect().foreach(tagLabel => trainModel(tagLabel, songDataList, sc))

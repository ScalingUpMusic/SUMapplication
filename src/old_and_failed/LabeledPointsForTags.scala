...
object OneTagModel {
    ...
    def trainModel(tag: string, args: Array[String], sc: SparkContext) {
        ...
        val rslt_save_path = hdfspath + model_dir + rslt_dir + tag
        ...
        val model_save_path = hdfspath + model_dir + model_subdir + tag + "_model.model"
        try { hdfs.delete(new org.apache.hadoop.fs.Path(model_save_path), true) } catch { case _ : Throwable => { } }
        model.save(sc, model_save_path)

    }

    def main(tag: string, args: Array[String], sc: SparkContext){
        val start_time = System.currentTimeMillis()/1000
        val conf = new SparkConf().setAppName("OneTagModel")
        val sc = new SparkContext(conf)
        val tagList = Array[String]("rock", "hip hop")
        val distTagList = sc.parallelize(tagList)
        distTagList.map(tag => trainModel(tag))
        println("Total time: " + (System.currentTimeMillis()/1000 - start_time))
    }

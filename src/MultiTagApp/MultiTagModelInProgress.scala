...
object MultiTagModel {
    ...
    def main(args: Array[String]){
        ...
        val tagList = Array[String]("rock", "hip hop", "country")
        println(tagList.mkString(" "))
        val conf = new SparkConf().setAppName("MultiTagModel")
        val sc = new SparkContext(conf)
        val distTagList = sc.parallelize(tagList)
        distTagList.collect().foreach(tag => println(tag))
        distTagList.collect().foreach(tag => trainModel(tag, sc))

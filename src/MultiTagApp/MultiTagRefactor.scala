...
object MultiTagModel {
    ...
    def trainModel(tagLabel: (String, Array[Double]), songData: Array[org.apache.spark.mllib.linalg.Vector]) : (org.apache.spark.mllib.classification.SVMModel, String) = {
        
        val sub_start_time = System.currentTimeMillis()/1000
        
        val tag = tagLabel._1
        val labels = tagLabel._2
        // Extract data from text file of the form ('TRACKID', (['tag1', 'tag2', ..., 'tagn'], [data1, data2, ..., datam]))
        val data_unscaled = labels zip songData
        if (verbose) {
        ...
        val model = new SVMWithSGD().setIntercept(model_intercept).run(train)
    ...
    def main(args: Array[String]){
        ...
        val allSongData = sc.textFile(loadpath)
        val array_string = "\\[.*?\\]".r
        val songArrays = allSongData.map(line => (array_string findAllIn line).toList)
        val songTagsAndData = songArrays.map(ars => (ars(0), Vectors.dense((ars(1) replaceAll ("[\\[\\]\\s]", "")).split(",").map(x => x.toDouble))))
        val songTagsList = songTagsAndData.map(_._1).collect()
        val songDataList = songTagsAndData.map(_._2).collect()
        val tagLabels = topTagsList.map(tag => (tag, songTagsList.map(songTags => if (songTags contains tag) 1.0 else 0.0)))
        val models = tagLabels.map(tagLabel => trainModel(tagLabel, songDataList))
        models.foreach(model => try { hdfs.delete(new org.apache.hadoop.fs.Path(model._2), true) } catch { case _ : Throwable => { } })
        models.foreach(model => model._1.save(sc, model._2))
